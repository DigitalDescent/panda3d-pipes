...
add_object(PyObject *dist_obj) {
  ...
  PyObject *py_do_id = PyObject_GetAttrString(dist_obj, (char *)"do_id");
  if (!py_do_id) {
    pipes_cat.error()
      << "add_object: dist_obj does not have a do_id member\n";
    return;
  }
  ...
  if (PyObject_HasAttrString(dist_obj, (char *)"pre_data_update")) {
    data->_pre_data_update = PyObject_GetAttrString(dist_obj, (char *)"pre_data_update");
  } else {
    data->_pre_data_update = nullptr;
  }
  if (PyObject_HasAttrString(dist_obj, (char *)"post_data_update")) {
    data->_post_data_update = PyObject_GetAttrString(dist_obj, (char *)"post_data_update");
  } else {
